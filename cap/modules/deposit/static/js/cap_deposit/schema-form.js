              ...
              if (form) {
                ...
                templatePromise.then(function(template) {
                  ...
                  if (form.condition) {
                    ...
                    var evalExpr = 'evalExpr(form.condition,{ model: model, "arrayIndex": arrayIndex, "arrayIndices": [arrayIndex]})';

                    if (form.key) {
                      var strKey = sfPathProvider.stringify(form.key);

                      arrayIndices = form.key.filter(function(e){return Number(e) === e;});

                      evalExpr = 'evalExpr(form.condition,{ model: model, "arrayIndex": $index, ' +
                                 '"arrayIndices": [' + arrayIndices + '],' +
                                 '"modelValue": model' + (strKey[0] === '[' ? '' : '.') + strKey + '})';
                    }
