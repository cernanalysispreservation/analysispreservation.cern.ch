...
class HTTPSClientAuthHandler(urllib2.HTTPSHandler):
    ...
    def __init__(self, key=None, cert=None, level=0):
        if  level > 1:
            urllib2.HTTPSHandler.__init__(self, debuglevel=1, context = ctx)
        else:
...
def get_data(host, query, idx, limit, debug, threshold=300, ckey=None,
    ...
    url += '?%s' % encoded_data
    req  = urllib2.Request(url=url, headers=headers, unverifiable=False)
    if  ckey and cert:
        ckey = fullpath(ckey)
        cert = fullpath(cert)
        http_hdlr  = HTTPSClientAuthHandler(ckey, cert, debug)
    else:
        http_hdlr  = urllib2.HTTPSHandler(debuglevel=debug, context=ctx)
