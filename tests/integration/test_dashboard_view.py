...
def test_dashboard_view_returns_user_drafts(app,
                                            users,
                                            create_deposit,
                                            auth_headers_for_user):
    my_deposit = create_deposit(users['lhcb_user'], 'lhcb-v0.2.0')
    user_published_deposit = create_deposit(users['lhcb_user'], 'lhcb-v0.2.0', publish=True)
...
def test_dashboard_view_returns_user_published_analysis(app,
                                                        users,
                                                        create_deposit,
                                                        auth_headers_for_user):
    my_deposit = create_deposit(users['lhcb_user'], 'lhcb-v0.2.0', experiment='LHCb')
    user_published_deposit = create_deposit(users['lhcb_user'], 'lhcb-v0.2.0', experiment='LHCb', publish=True)
    other_user_deposit = create_deposit(users['lhcb_user2'], 'lhcb-v1.2.0', experiment='LHCb')
    other_user_published_deposit = create_deposit(users['lhcb_user2'], 'lhcb-v0.2.0',
                                                  experiment='LHCb', publish=True)
...
def test_dashboard_view_returns_published_by_collaboration(app,
                                                           users,
                                                           create_deposit,
                                                           auth_headers_for_user):
    my_deposit = create_deposit(users['lhcb_user'], 'lhcb-v0.2.0',
    ...
    user_published_deposit = create_deposit(users['lhcb_user'], 'lhcb-v0.2.0', publish=True, 
                                            experiment='LHCb')
    published_by_member_of_collab = create_deposit(users['lhcb_user2'], 'lhcb-v1.2.0', publish=True,
                                                   experiment='LHCb')
    published_by_another_collab = create_deposit(users['cms_user'], 'cms-v1.2.0', publish=True,
                                                 experiment='CMS')
    ...
    with app.test_client() as client:
        ...
        assert published_by_member_of_collab['control_number'] in published_by_collab
        assert published_by_another_collab['control_number'] not in published_by_collab
...
def test_dashboard_view_returns_published_by_collaboration(app,
    ...
    my_deposit = create_deposit(users['lhcb_user'], 'lhcb-v0.2.0', experiment='LHCb')
    user_published_deposit = create_deposit(users['lhcb_user'], 'lhcb-v0.2.0', publish=True) 
    published_by_member_of_collab = create_deposit(users['lhcb_user2'], 'lhcb-v0.2.0', publish=True)
    published_by_another_collab = create_deposit(users['cms_user'], 'cms-v0.2.0', publish=True)

    with app.test_client() as client:
        resp = client.get('/dashboard', headers=auth_headers_for_user(users['lhcb_user']))
        published_by_collab = [x['id'] for x in resp.json['published_by_collab']]
...
def test_dashboard_view_returns_shared_with_user(action,
                                                 app,
                                                 users,
                                                 create_deposit,
                                                 json_headers,
    ...
    with app.test_client() as client:
        client.post('/deposits/{}/actions/permissions'.format(other_user_deposit['_deposit']['id']),
...
def test_dashboard_view_returns_user_counts_properly(app,
                                                     users,
                                                     create_deposit,
                                                     auth_headers_for_user):
