...
def test_get_deposits_doesnt_return_published_ones(app, db, users,
    ...
    with app.test_client() as client:
        ...
        assert resp.status_code == 200
        assert len(hits) == 1
        assert deposit.pid.pid_value in [
            hit['metadata']['_deposit']['id'] for hit in hits]
...
def test_get_deposits_returns_deposits_that_user_has_read_or_admin_access_to(action, app, db, users,
    ...
    with app.test_client() as client:
        ...
        resp = client.get(
            '/deposits/', headers=auth_headers_for_user(other_user))
        hits = resp.json['hits']['hits']
...
def test_get_deposits_with_basic_json_serializer_returns_serialized_deposit_properly(app,
    ...
    user = users['cms_user']
    deposit = create_deposit(user, 'cms', {
        '$schema': 'https://{}/schemas/deposits/records/cms-v1.0.0.json'.format(jsonschemas_host),
        'basic_info': {
...
def test_get_deposit_with_basic_json_serializer_returns_serialized_deposit_properly(app,
                                                                                    ...
                                                                                    auth_headers_for_superuser,
                                                                                    create_deposit):
    deposit = create_deposit(users['superuser'], 'cms', {
        '$schema': 'https://{}/schemas/deposits/records/cms-v1.0.0.json'.format(jsonschemas_host),
        'basic_info': {
            'analysis_number': 'dream_team',
            'people_info': [{}]
