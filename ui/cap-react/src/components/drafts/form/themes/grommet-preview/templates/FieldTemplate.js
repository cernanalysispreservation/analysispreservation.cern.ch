...
let FieldTemplate = function (props) {
  ...
  return children[0].props &&
    children[0].props.formData === undefined ? null : (
      ...
      >
        {label && !(["array"].indexOf(props.schema.type) > -1) || props.schema.uniqueItems ? (
          <Box
            flex
            style={{
              paddingRight: "10px",
              gridColumn: ["array", "object"].indexOf(props.schema.type) > -1 ?
              (props.schema.uniqueItems ? "1/3" : "1/5") :
              (label ? "1/3" : "1/5")
            }}
          >
        ...
        <Box flex justify="center"
          style={{
            gridColumn: ["array", "object"].indexOf(props.schema.type) > -1 ?
              (props.schema.uniqueItems ? "3/5" : "1/5") :
              (label ? "3/5" : "1/5")
          }}>
          {children}
        </Box>
