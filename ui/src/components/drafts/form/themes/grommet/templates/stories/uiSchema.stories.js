...
/**
 * Parameters:
 * The developers can define the layout of the form by passing three parameters (uiSchema) to the root object, and more specific to "ui:options"
 * 
 * First of all in order to achieve the tabView Layout, have to add to the uiSchema to the root object, "ui:object":"tabView"
 * 
 * Therefore they have to provide the following object:
 * "ui:options":{
      full:true,
      view:{
        vertical:true,
 ...
 *    
 * The full patameter: 
 * defines whether ta tabView will be in full or half width.
 * Accepted values are true or false. Default value is true
 * 
 * The view object defines:
 * The vertical parameter: defines whether the tab will be as column(vertical = true ) on the left
 *                        or whether it will be as a row (vertical =false) on the top
 * The sidebarColor defines: the background color of the tab
 * 
 * 
 * 
 * The default display for the children is grid and defines 4 equally splitted columns.
 * Every children needs to define how many calumns requires in order to be displayed. 
 * This provides the choice to contain from one ro four different fields at a row.
 * 
 * In order to specify how many columns a field has to fill, the developers has to pass at the "ui:options" object the following
 * "ui:options":{
    "grid":{
      "gridColumn":"1 / 2"
      }
 ...
 *
 * The above example shows that this specific field will start at the first column and will end at the second.
 * The first number defines the start column and the second the end column. 
 * If the assigned number of columns are more than the limits it will continue to the next line and apply the rule
 * Minimum:1, Maximum: 5
 * 
 * The default value for the gridColumns is 1 / 5, theefore one field per row.
 * */

class UiSchemaStorie extends Component {
  constructor(props) {
    super(props);
    this.state = {};
  }
...
storiesOf("UI Schema", module)
  ...
  // .add('Object Field in Grid',() => <UiSchemaStorie uiSchema={objschemaui1} schema={objschema}/>)
  .add("Full Object Field in Grid", () => (
    <UiSchemaStorie uiSchema={objschemaui1} schema={objschema} />
  ))
  .add("Center Object Field in Grid", () => (
    <UiSchemaStorie uiSchema={testSchemaUi} schema={testSchema} />
  ))
  .add("CMS TabView", () => (
    <UiSchemaStorie schema={schemaCMS} uiSchema={schemaCMSUI} />
  ))

  .add("ATLAS TabView", () => (
    <UiSchemaStorie schema={schemaATLAS} uiSchema={schemaATLASUI} />
  ))

  .add("LHCB TabView", () => (
    <UiSchemaStorie schema={schemaLHCB} uiSchema={schemaLHCBUI} />
  ));
